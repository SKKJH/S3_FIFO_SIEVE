...
cache_obj_t *cache_insert_base_num(cache_t *cache, const request_t *req) {
...
cache_obj_t *cache_insert_base(cache_t *cache, const request_t *req) {
  ...
  if (cache->type == 1) {
    // printf("imhere999999999\n");
    if (cache->first_flag == 1) {
      // printf("imhere99999\n");
      cache->ocp_flag_size += (int64_t)cache_obj->obj_size + (int64_t)cache->obj_md_size;
      if ((cache->ocp_flag_size * 1.0) > (cache->cache_size * (1.0 / 3.0))) {
        if ((cache->t_00 == NULL) && (cache->t_33 == NULL) && (cache->t_66 == NULL)) {
          cache->t_00 = cache_obj;
        } else if ((cache->t_33 == NULL) && (cache->t_66 == NULL)) {
          cache->t_33 = cache->t_00;
          cache->t_00 = cache_obj;
        } else {
          cache->t_66 = cache->t_33;
          cache->t_33 = cache->t_00;
          cache->t_00 = cache_obj;
          cache->first_flag = 0;
        }
      ...
      cache->t_33 = cache->t_33->queue.prev;
